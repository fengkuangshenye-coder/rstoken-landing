...
$dirFeature  = J $ProjectRoot 'src\features\referral'
$dirPages    = J $ProjectRoot 'src\pages'
$dirAppMe    = J $ProjectRoot 'src\app\me'
$dirs = @($dirLib,$dirFeature,$dirPages,$dirAppMe)
...
'@
Set-Content (J $dirFeature 'referral.ts') $referralTs -Encoding UTF8

# MePage.tsx（带 “Me v2 ✅” 角标，确认是否命中）
$mePageTsx = @'
...
'@
Set-Content (J $dirFeature 'MePage.tsx') $mePageTsx -Encoding UTF8

# Pages Router 入口（src/pages 与 pages 各写一份，谁匹配谁生效）
Ensure-Dir (J $ProjectRoot 'pages')
Set-Content (J $ProjectRoot 'pages\me.tsx') "import MePage from '../src/features/referral/MePage'; export default MePage;" -Encoding UTF8
Set-Content (J $dirPages 'me.tsx')          "import MePage from '../features/referral/MePage'; export default MePage;" -Encoding UTF8

# App Router 入口（可并存）
Ensure-Dir $dirAppMe
Set-Content (J $dirAppMe 'page.tsx') "'use client'; import MePage from '../../features/referral/MePage'; export default function Page(){ return <MePage/> }" -Encoding UTF8
...
"`n✅ 已写入：tokens.ts / referral.ts / MePage.tsx 以及 /me 路由入口。请重启 dev 服务后访问 /me，顶部会显示 'Me v2 ✅' 作为命中标识。" | Write-Host
