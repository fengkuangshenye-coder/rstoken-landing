...
function J([string]$p1,[string]$p2){ Join-Path -Path $p1 -ChildPath $p2 }
function Ensure-Dir($p){ if(-not (Test-Path $p)){ New-Item -ItemType Directory -Force -Path $p | Out-Null } }

# 1) 目录
$dirLib          = J $ProjectRoot 'src\lib'
$dirReferral     = J $ProjectRoot 'src\features\referral'
$dirSrcPages     = J $ProjectRoot 'src\pages'
$dirRootPages    = J $ProjectRoot 'pages'
$dirSrcAppMe     = J $ProjectRoot 'src\app\me'
$dirRootAppMe    = J $ProjectRoot 'app\me'

($dirLib,$dirReferral,$dirSrcPages,$dirRootPages,$dirSrcAppMe,$dirRootAppMe) | % { Ensure-Dir $_ }
...
export const TOKENS: Record<number, {
  ...
  [97]: {
    ...
    USDT: { address: "0x0000000000000000000000000000000000000000", decimals: 18 }, // TODO 替换测试网 USDT
...
export const TOTAL_RATE = REF_SPLIT.L1 + REF_SPLIT.L2 + REF_SPLIT.L3;
...
export function getMyInviteLink(addr?:Address){ if(typeof window==="undefined") return ""; const base=`${location.origin}${location.pathname}`; if(!addr) return ""; return `${base}?ref=${addr}`; }
export function getUplines(of:Address){ const refOf=parse<Record<Address,Address>>(LS_KEYS.REF_OF,{} as any); const l1=refOf[of]; const l2=l1?refOf[l1]:undefined; const l3=l2?refOf[l2]:undefined; return {l1,l2,l3}; }
export function recordContributionUSDT(buyer:Address,amountUSDT:number){
  ...
  const add=(who?:Address,part?:"l1"|"l2"|"l3",rate?:number)=>{ if(!who||!rate) return; rewards[who]=(rewards[who]||0)+amountUSDT*rate; stats[who]=stats[who]||{l1:0,l2:0,l3:0}; if(part) stats[who][part]+=amountUSDT; };
...
export function getMyRewards(addr?:Address){ const a=norm(addr || parse<Address>(LS_KEYS.ME,"" as any)); const rewards=parse<Record<Address,number>>(LS_KEYS_REWARDS,{} as any); const stats=parse<Record<Address,{l1:number;l2:number;l3:number;}>>(LS_KEYS.STATS,{} as any); return { pendingUSDT:Number((rewards[a]||0).toFixed(6)), stats:stats[a]||{l1:0,l2:0,l3:0} }; }
export function withdrawMyRewards(addr?:Address){ const a=norm(addr || parse<Address>(LS_KEYS.ME,"" as any)); const rewards=parse<Record<Address,number>>(LS_KEYS.REWARDS,{} as any); if(rewards[a]){ rewards[a]=0; save(LS_KEYS.REWARDS,rewards); } }
'.Replace('LS_KEYS_REWARDS','LS_KEYS.REWARDS')  # 修正拼写
...
# 4) 路由入口：把所有可能位置都指向同一组件（谁匹配，谁生效）
Set-Content -Path (J $dirSrcPages 'me.tsx') -Value "import MePage from '../features/referral/MePage'; export default MePage;" -Encoding UTF8
Set-Content -Path (J $dirRootPages 'me.tsx') -Value "import MePage from '../src/features/referral/MePage'; export default MePage;" -Encoding UTF8

Ensure-Dir (J $ProjectRoot 'pages\me')
Set-Content -Path (J $ProjectRoot 'pages\me\index.tsx') -Value "import MePage from '../../src/features/referral/MePage'; export default MePage;" -Encoding UTF8

Set-Content -Path (J $dirSrcAppMe 'page.tsx') -Value "'use client'; import MePage from '../../features/referral/MePage'; export default function Page(){ return <MePage/> }" -Encoding UTF8
Set-Content -Path (J $dirRootAppMe 'page.tsx') -Value "'use client'; import MePage from '../../src/features/referral/MePage'; export default function Page(){ return <MePage/> }" -Encoding UTF8
...
foreach($line in $envLines){ if($existing -notmatch [regex]::Escape($line)){ Add-Content -Path $envPath -Value $line } }

# 6) 打印项目中所有可能的 /me 路由文件，便于你确认（只打印，不删除）
"`n🔎 已写入以下路由文件（按优先级不同，Next 会命中其中之一）："
(Get-ChildItem -Path $ProjectRoot -Recurse -Include 'me.tsx','index.tsx' | Where-Object { $_.FullName -match '\\(src\\)?(pages|app)\\(me|me\\index)\.tsx$' }) |
  Select-Object FullName,Length,LastWriteTime |
  Format-Table | Out-String | Write-Host

"`n✅ 修复完成。重启开发服务后访问 http://localhost:3000/me ，页面顶部应看到 'Me v2 ✅' 标识。" | Write-Host
