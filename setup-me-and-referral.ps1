param(
  [string]$ProjectRoot = "D:\RStoken5.1.2"
)

# --- 准备目录 ---
$paths = @(
  Join-Path $ProjectRoot "src\lib",
  Join-Path $ProjectRoot "src\features\referral",
  Join-Path $ProjectRoot "src\pages",
  Join-Path $ProjectRoot "src\app\me"
)
foreach ($p in $paths) { New-Item -ItemType Directory -Force -Path $p | Out-Null }

# --- 写入 src/lib/tokens.ts ---
$tokensTs = @'
export const CHAINS = {
  BSC_MAINNET: 56,
  BSC_TESTNET: 97,
...
'@
Set-Content -Path (Join-Path $ProjectRoot "src\lib\tokens.ts") -Value $tokensTs -Encoding UTF8

# --- 写入 src/features/referral/referral.ts ---
$referralTs = @'
...
'@
Set-Content -Path (Join-Path $ProjectRoot "src\features\referral\referral.ts") -Value $referralTs -Encoding UTF8

# --- 写入 src/features/referral/MePage.tsx（页面主体） ---
$mePageTsx = @'
...
'@
Set-Content -Path (Join-Path $ProjectRoot "src\features\referral\MePage.tsx") -Value $mePageTsx -Encoding UTF8

# --- 写入 src/pages/me.tsx（Pages Router 版本） ---
$pagesMe = @'
import MePage from "../features/referral/MePage";
export default MePage;
'@
Set-Content -Path (Join-Path $ProjectRoot "src\pages\me.tsx") -Value $pagesMe -Encoding UTF8

# --- 写入 src/app/me/page.tsx（App Router 版本） ---
$appMe = @'
"use client";
import MePage from "../../features/referral/MePage";
export default function Page(){ return <MePage/> }
'@
Set-Content -Path (Join-Path $ProjectRoot "src\app\me\page.tsx") -Value $appMe -Encoding UTF8

# --- 补全 tsconfig 路径别名 "@/ *"（若存在 tsconfig.json） ---
...
if (Test-Path $tsconfigPath) {
  try {
    ...
    Write-Host "⚠️ tsconfig.json 更新失败（已跳过）：$($_.Exception.Message)"
  }
}
...
if (Test-Path $envPath) {
  foreach($line in $envLines){
    if (-not (Select-String -Path $envPath -Pattern [regex]::Escape($line) -SimpleMatch -Quiet)){
      Add-Content -Path $envPath -Value $line
    }
  }
} else {
  Set-Content -Path $envPath -Value ($envLines -join "`n") -Encoding UTF8
}

Write-Host "✅ 已完成：余额展示 + 三级邀请分润前端（演示版）文件写入。"
Write-Host "👉 请在 .env.local 填上你的 RST 合约地址；如需测试网，也请替换 USDT 测试网地址。"
Write-Host "👉 进入项目目录启动：pnpm dev  或  npm run dev"
